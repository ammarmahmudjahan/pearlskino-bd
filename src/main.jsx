...
const F = [
  [
    "How fast do you deliver in Dhaka?",
    "Orders inside Dhaka are dispatched through local courier partners. Your exact ETA is confirmed at checkout.",
  ],
  [
    "Do you deliver nationwide in Bangladesh?",
    "Yes. Nationwide delivery is supported. Delivery time and charge depend on destination.",
  ],
  [
    "Are your products authentic?",
    "PearlSkino BD is positioned around authentic products with sourcing transparency.",
  ],
  [
    "Can I pay online?",
    "Yes. The checkout is structured for SSLCOMMERZ, with merchant credentials kept server-side.",
  ],
  [
    "Can I change or cancel an order?",
    "Contact PearlSkino BD as soon as possible; changes depend on order status.",
  ],
];
...
function App() {
  const [cart, setCart] = useState(() =>
    JSON.parse(localStorage.getItem("pearlskino-cart") || "[]")
  );
  ...
  useEffect(() => {
    localStorage.setItem("pearlskino-cart", JSON.stringify(cart));
  }, [cart]);

  useEffect(() => {
    let i = setInterval(() => setEnd((x) => Math.max(0, x - 1000)), 1000);
    return () => clearInterval(i);
  }, []);

  useEffect(() => {
    if (!toast) return;
    let i = setTimeout(() => setToast(""), 2200);
  ...
  return (
    <div>
      ...
      <div className="topbar">
        <Sparkles />
        100% authentic products • Free pickup in selected areas • Nationwide
        delivery
      </div>
      <header className="header">
        <div className="nav">
          ...
          <div className="actions">
            <button className="icon" onClick={() => setQuery(query ? "" : " ")}>
              <Search />
            </button>
            <button className="icon desktop">
              <CircleUserRound />
            </button>
      ...
      <main>
        <section className="hero">
          <div className="liquid a" />
          <div className="liquid b" />
          <div className="liquid c" />
          <div className="hero-inner">
            <motion.div {...rev}>
              ...
              <p>
                Curated skincare, cosmetics and fragrances — thoughtfully chosen
                for your everyday ritual.
              </p>
              <div className="ctas">
                <button
                  className="btn dark"
                ...
                <a className="btn glass" href="#collections">
                  Explore Fragrances
                </a>
              </div>
              <div className="proof">
        ...
        <section className="sale-wrap">
          <div className="sale">
            ...
            <div className="count">
              {[
                [Math.floor(end / 3600000), "H"],
                [Math.floor((end % 3600000) / 60000), "M"],
                [Math.floor((end % 60000) / 1000), "S"],
              ].map((x, idx) => (
                <div key={idx}>
                  <b>{String(x[0]).padStart(2, "0")}</b>
                  <span>{x[1]}</span>
                </div>
              ))}
            </div>
        ...
        <section id="shop" className="section">
          <motion.div {...rev} className="heading">
            <div>
              ...
              <p>
                Discover everyday essentials and statement scents selected for
                the PearlSkino community.
              </p>
            </div>
          ...
          <div className="filters">
            {["All", "Fragrance-Free", "Oily Skin", "EDP Perfumes"].map((f) => (
              <button
                key={f}
          ...
          <div className="grid">
            {list.map((p) => (
              <Product key={p.id} p={p} add={add} open={setModal} />
            ))}
          </div>
        </section>
        <section id="collections" className="section">
          ...
          <div className="categories">
            {[
              ["Fragrance", "Signature scents & decants", P[0].image],
              ["Skincare", "Simple, effective essentials", P[3].image],
              ["Cosmetics", "Everyday glow, elevated", P[5].image],
            ].map((x, idx) => (
              <a key={idx} className="category" href="#shop">
                <img src={x[2]} alt={x[0]} />
                <div>
                  <small>{x[0]}</small>
                  <b>{x[1]}</b>
        ...
        <section id="story" className="story">
          ...
          <motion.div {...rev}>
            ...
            <p>
              We believe beauty shopping should feel personal, transparent and a
              little magical. PearlSkino BD brings fragrances, skincare and
              cosmetics into one carefully edited destination.
            </p>
            <div className="benefits">
              <div>
                ...
                <b>
                  ...
                  <small>We put product trust first.</small>
                </b>
              </div>
              <div>
                <PackageCheck />
                <b>
                  Thoughtful packing
                  <small>Every order deserves care.</small>
                </b>
              </div>
              <div>
                <Truck />
                <b>
                  Made for Bangladesh
                  <small>Delivery designed around BD.</small>
                </b>
              </div>
            </div>
          </motion.div>
        ...
        <section className="section">
          ...
          <div className="reviews-grid">
            {[
              [
                "“The whole experience feels much more premium than I expected.”",
                "Sadia R.",
              ],
              [
                "“The decants are perfect when I want to test a scent before committing.”",
                "Nabila T.",
              ],
              [
                "“Clean, easy to browse and the product presentation is beautiful.”",
                "Fahim M.",
              ],
            ].map((x, idx) => (
              <div key={idx} className="review">
                <div>★★★★★</div>
                <blockquote>{x[0]}</blockquote>
                <small>— {x[1]}</small>
              </div>
            ))}
          </div>
            ...
            {!cart.length ? (
              ...
              <>
                ...
                <div className="summary">
                  ...
                  <p>
                    Delivery <b>{ship ? money(ship) : "FREE"}</b>
                  </p>
                  <h4>
                    Total <b>{money(total)}</b>
                  </h4>
                  <a className="checkout" href="/checkout.html">
                    Proceed to secure checkout <ArrowRight />
                  </a>
                </div>
              </>
            )}
      ...
      {p && (
        <>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <button className="modalx" onClick={close}>
              <X />
            </button>
            <img src={p.image} alt={p.name} />
            <div>
